...
for (const экран of ЭКРАНЫ) {
  ...
  for (const стр of СТРАНИЦЫ) {
    ...
    try {
      ...
      // Ждём именно готовности шрифтов, а не просто «ещё полторы секунды».
      // Пока подставной шрифт не сменился на настоящий, кнопки и ссылки
      // на доли пикселя ниже нормы — и проверка один раз показала ложное
      // «мелкое нажатие 44x44» на главной магазина, которого при повторном
      // прогоне не было. Проверка, которая иногда врёт, хуже отсутствующей:
      // её замечания перестают читать.
      await страница.evaluate(() => document.fonts && document.fonts.ready)
        .catch(() => {});
