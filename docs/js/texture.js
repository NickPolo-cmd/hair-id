...
(function () {
  ...
  function initParallax(layers) {
    ...
    function кадр() {
      ...
      // Раскладку в кадре не читаем вовсе.
      //
      // Сначала здесь чтение и запись чередовались в одном цикле: каждая
      // запись отменяла посчитанную раскладку, и следующее
      // getBoundingClientRect заставляло браузер считать её заново — по
      // одному принудительному пересчёту на слой, на каждом кадре прокрутки.
      // Тогда чтение вынесли в отдельный проход, и стало лучше: средний кадр
      // 56 мс вместо 16. Но пересчёт всё равно оставался — девять запросов
      // рамки на кадр, а каждый из них считает раскладку всей страницы.
      // Теперь место слоя снимается один раз (снятьМеста) и обновляется
      // только когда раскладка действительно поменялась: размер окна,
      // поворот экрана, новая фактура. В кадре остаётся одна арифметика.
      var сдвигОкна = window.scrollY || window.pageYOffset || 0;

      for (var j = 0; j < layers.length; j++) {
        var l = layers[j];
        var верх = l.docTop - сдвигОкна;
        if (верх + l.height < -200 || верх > vh + 200) continue;
        // -1 когда секция входит снизу, +1 когда уходит вверх
        var ход = ((vh - верх) / (vh + l.height)) * 2 - 1;
    ...
    // Места слоёв снимаются один раз и пересчитываются при изменении раскладки.
    function снятьМеста() {
      var сдвигОкна = window.scrollY || window.pageYOffset || 0;
      for (var i = 0; i < layers.length; i++) {
        var р = layers[i].host.getBoundingClientRect();
        layers[i].docTop = р.top + сдвигОкна;
        layers[i].height = р.height;
      }
    }
    ...
    // Фактуры и картинки догружаются позже и двигают всё, что ниже них.
    window.addEventListener('load', function () { снятьМеста(); приПрокрутке(); });
    if (document.fonts && document.fonts.ready) {
      document.fonts.ready.then(function () { снятьМеста(); приПрокрутке(); });
    }
    кадр();
  }
