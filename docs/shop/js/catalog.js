...
(function () {
  ...
  // Список-образец убран. Он был написан до появления настоящего каталога
  // и с тех пор жил мёртвым грузом: 158 строк с чужими идентификаторами
  // (sh-keratin-shampoo вместо t005), которые ни к чему не привязаны. Наборы
  // ссылаются на настоящие id, поэтому при неудачной загрузке данных карточки
  // набора отрисовывались пустыми, а в заказ уходило «Набор ... = цену уточню»
  // без содержимого — хуже, чем честное «каталог недоступен».
  var PRODUCTS = [];
  ...
  function renderProducts() {
    ...
    if (!PRODUCTS.length) {
      // Файл с товарами не загрузился. Честно говорим об этом, а не делаем вид,
      // что под задачу ничего не нашлось.
      box.innerHTML = '<p class="shop-empty">Каталог сейчас не открывается. ' +
        'Напишите в WhatsApp или Telegram — подберу и пришлю список.</p>';
      return;
    }
    ...
    box.innerHTML = list.map(function (p) {
      ...
      '<article class="product' + inCart + '" id="' + экр(p.id) + '" data-product="' + экр(p.id) + '" data-card-link>' +
        ...
        '<div class="product__body">' +
          ...
          afterProcedure(p) +
          priceList(p) +
          // Подпись видимая одна и та же у всех карточек — по ней человек
          // сразу понимает, что внутри. Но читалка экрана перечисляет
          // раскрывашки списком, и тридцать шесть одинаковых «Почему это
          // работает» подряд не различить. Название товара добавляем
          // только для читалки, вид карточки не меняется.
          '<details class="product__more"><summary aria-label="Почему это работает: ' +
            экр(p.name) + '">Почему это работает</summary>' +
  ...
  function renderCart() {
    ...
    if (panel) {
      // Если человек закрыл панель крестиком, она не должна выезжать обратно
      // от любого пересчёта — например, когда он просто выбирает объём
      // в карточке. Класс is-dismissed ставит shop-ui.js при закрытии и снимает
      // при добавлении новой позиции.
      var закрытаЧеловеком = panel.classList.contains('is-dismissed');
      panel.classList.toggle('is-visible', n > 0 && !закрытаЧеловеком);
    ...
    var msg = buildOrder();
    if (wa) {
      // Кириллица в адресе занимает по девять знаков на букву: полный заказ
      // из 36 позиций давал ссылку в 16 тысяч знаков. Длинный заказ шлём
      // коротким перечнем, а полный текст кладём на страницу для копирования.
      var длядлины = msg.length > 1500 ? краткийЗаказ() : msg;
      if (длядлины !== msg) показатьТекст(msg);
      wa.href = 'https://wa.me/' + PHONE + '?text=' + encodeURIComponent(длядлины);
    }
  ...
  // Буфер обмена недоступен по http, в старых браузерах и при отказе
  // в разрешении. Раньше в этом случае текст заказа пропадал совсем:
  // человек уходил в мессенджер с пустым чатом и подписью «скопируйте
  // вручную», а копировать было нечего. Теперь текст показывается на странице,
  // выделенным — остаётся нажать «копировать» самому.
  // Короткий вид заказа: только название и количество, без пометок.
  function краткийЗаказ() {
    var строки = ['Здравствуйте! Хочу заказать:'];
    Object.keys(cart).forEach(function (id) {
      var line = lineById(id);
      if (line) строки.push('— ' + line.title + ' × ' + cart[id]);
    });
    строки.push('Полный список пришлю следующим сообщением.');
    return строки.join('\n');
  }

  function показатьТекст(текст) {
    var панель = document.getElementById('cart-panel');
    if (!панель) return;
    var поле = панель.querySelector('.cart-fallback');
    if (!поле) {
      поле = document.createElement('textarea');
      поле.className = 'cart-fallback';
      поле.setAttribute('readonly', 'readonly');
      поле.setAttribute('aria-label', 'Текст заказа — скопируйте его сами');
      панель.appendChild(поле);
    }
    поле.value = текст;
    поле.hidden = false;
    try { поле.focus(); поле.select(); } catch (e) { /* не вышло — текст всё равно виден */ }
  }
  ...
  function initTelegram() {
    ...
    tg.addEventListener('click', function () {
      ...
      var текст = buildOrder();
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(текст).then(function () { done(true); }, function () { показатьТекст(текст); done(false); });
      } else {
        показатьТекст(текст);
        done(false);
      }
