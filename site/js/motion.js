...
(function () {
  ...
  // Ширина окна на прошлом замере. Повторный замер при той же ширине
  // ничего не изменит, а стоит он дорого: на время замера шапке
  // приписывается состояние «страница прокручена», и хотя переходы
  // выключаются, сам класс на body переставляется туда и обратно.
  // Замер зовётся после загрузки, после шрифтов и после каждой новой
  // фактуры — на главной это тринадцать переключений класса за две
  // секунды загрузки. Лишние отсекаем.
  var ширинаЗамера = -1;

  function measureLogoMorph(принудительно) {
    if (!logoMorph) return;
    var ш = window.innerWidth;
    if (!принудительно && ш === ширинаЗамера) return;
    ширинаЗамера = ш;
  ...
  function init() {
    ...
    if (document.fonts && document.fonts.ready) {
      document.fonts.ready.then(function () {
        // Шрифт меняет ширину надписи при той же ширине окна — здесь
        // замер обязателен, поэтому просим принудительно.
        measureLogoMorph(true);
